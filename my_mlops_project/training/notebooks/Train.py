...
# MAGIC # %pip install -r ../../requirements.txt # (Tmp Solved) Disabled pip install to use serverless, (Info) This line will throw error if run on CE account, as serverless CE image is Python 3.12, but requirements.txt may required other versions. Therefore, we will use Trial account to create a cluster and use it instead of serverless.
